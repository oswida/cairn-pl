...
# Berserker
4 OCHR, 1 Pancerza, 14 SIŁ, battleaxe (k10, bulky)  
...
# Black Pudding
10 OCHR, 5 ZRE, devouring touch (k8)  
...
- For each detachment of regular brigands, there’s one leader who has chainmail (2 Pancerza) and a longsword (k8).  

autor: [xenio](https://xenioinabottle.blogspot.com)

# Buccaneer
4 OCHR, 14 ZRE, 12 WOL, scimitar (k6)  
...
# Bugbear
4 OCHR, 1 Pancerza, 14 ZRE, club (k8, bulky)  
...
- Critical damage: the tail renders the victim stunned (k8 ZRE damage).  
...
# Centaur
4 OCHR, 1 Pancerza, 12 SIŁ, 12 ZRE, 13 WOL, spear or short bow (k6)  
...
- Critical damage: their deadly poison hinders the target incapacitated for up to 10 days (save SIŁ once a day to recover).  
...
- The draconic head breaths fire in a cone for k10 damage.  
...
# Crab, Giant
3 OCHR, 3 Pancerza, 11 SIŁ, 4 ZRE, pincers (k8)  
...
- Can easily destroy smaller ships with their crushing zęby.  
...
# Crypt Thing
12 OCHR, 3 Pancerza (vs non-magical), 8 SIŁ, 11 ZRE, 14 WOL, ethereal pazury (1d8)
...
# Cyclops
13 OCHR, 18 SIŁ, 8 ZRE, 6 WOL, club (k10)  
...
# Demon Knight
10 OCHR, 3 Pancerza, 16 SIŁ, 8 ZRE, 12 WOL, longsword (k8), Unholy Symbol (Hatred when bathed in blood)  
...
# Devil Swine
9 OCHR, 16 SIŁ, 8 ZRE, gore (k6+k6)
...
# Djinn
10 OCHR, 1 Pancerza, 15 ZRE, fists (k8)  
...
# Dragon, Black
16 OCHR, 1 Pancerza, 13 SIŁ, 18 ZRE, 14 WOL, zęby (k10), pazury (k8+k8), _detachment_
...
- Any critical damage saves provoked by the dragon's zęby automatically fail, due to its powerful acidic saliva.   
...
- Breathes lightning in a long line for k10 damage to all caught.  
...
- Breathes lightning on a line for k10 damage or repulsion gas in a cone, pushing victims away.  
...
- Breathes acid in a cone for k10 damage or slowing gas, halving the movement of all affected.  
...
- Breath fire in a massive cone for k10 damage to all caught.  

autor: [xenio](https://xenioinabottle.blogspot.com)

# Dragon, Green
14 OCHR, 2 Pancerza, 14 SIŁ, 16 ZRE, 18 WOL, zęby (k10), pazury (k6+k6 _blast_), _detachment  
...
- Thunder Clap: The dragon releases the lightning coalesced in its mouth and pazury in a large explosion centered on itself, dealing k12 blast damage and knocking prone any target hit. It needs a short rest before being able to do this again.  

autor: [xenio](https://xenioinabottle.blogspot.com)

# Dragon, Red
18 OCHR, 3 Pancerza, 18 SIŁ, 12 ZRE, 16 WOL, zęby (k12), pazury (k8+k8), _detachment_  
...
- Fire Breath: The dragon breathes a massive cone of pure flame dealing 12 damage (no roll) to all caught, the flame ignores and destroys any mundane Pancerza. It needs a short rest before being able to do this again.

autor: [xenio](https://xenioinabottle.blogspot.com)

# Dragon, Silver
13 OCHR, 3 Pancerza, 16 SIŁ, 14 WOL, zęby (k8+k8)  
- Noble and selfless dragons, with shining scales rescinding of a knight's Pancerza.   Dwell at the top of cold mountains.  
- Frequently deals with humans, going through great efforts to help those in need.  
- Breathe a line of cold dealing k10 damage and potentially paralyzing those caught.  
...
- Breathes large clouds of steam, clogging visions and damaging all inside for k8 damage.  

autor: [xenio](https://xenioinabottle.blogspot.com)

# Dragon, White
16 OCHR, 1 Pancerza, 16 SIŁ, 16 ZRE, 12 WOL, zęby (k10), pazury (k8+k8), _detachment_
...
- It's always surrounded by a storm of ice and cold wind, dealing k6 blast damage every round to everything nearby, in addition to its normal attacks. 

autor: [xenio](https://xenioinabottle.blogspot.com)

# Draugr
12 OCHR, 2 Pancerza, 15 SIŁ, 9 ZRE, 13 WOL, rusty broadsword (k8)
...
- Upon hearing its name, the Draugr must make a WOL save or return to corpse-form.
...
- Critical Damage: The poison paralyzes the victim’s body (k6 ZRE damage).  

autor: [xenio](https://xenioinabottle.blogspot.com)

# Drow
4 OCHR, 8 SIŁ, 14 ZRE, 12 WOL, shortsword (k6), a Spellbook (choose one: Web, Spider Climb, or Night Sphere)  
...
# Duergar
4 OCHR, 1 Pancerza, 14 SIŁ, 8 WOL, Warhammer (k10, bulky)  
...
# Dwarf
4 OCHR, 2 Pancerza, 14 SIŁ, hammer (k8)  
...
# Efrreti
10 OCHR, 15 SIŁ, 14 WOL, flaming sword (k10, bulky)  
...
# Elemental, Air
16 OCHR, 11 SIŁ, 15 ZRE, 8 WOL, wind blow (k8)  
...
# Elemental, Earth
12 OCHR, 3 Pancerza, 16 SIŁ, 8 ZRE, fist (k10)  
...
# Elemental, Water
12 OCHR, 2 Pancerza, 15 SIŁ, blow (k8)  
...
# Elf
4 OCHR, 1 Pancerza, 8 SIŁ, 14 ZRE, 14 WOL, shortswords (k6+k6) or longbow (k8), a Spellbook (choose one: Charm or Detect Magic)  
...
# Estrie
10 OCHR, 11 SIŁ, 15 ZRE, 14 WOL, draining hair (1d8)
...
# Flail Snail
4 OCHR, 2 Pancerza, 14 SIŁ, 6 ZRE, tentacles (k8+k8)  
...
# Foxwoman
6 OCHR, 12 SIŁ, 14 ZRE, 11 WOL, teeth (k6), pazury (k8+k8)
...
# Frost Elf
14 OCHR, 1 Pancerza, 8 SIŁ, 13 ZRE, 14 WOL, icicle sztylet (k6), a Spellbook (Choose one: Sleep, Teleport, Detect Magic)  
...
# Gelatinous Cube
6 OCHR, 16 SIŁ, 8 ZRE, 4 WOL, touch (k8)  
...
# Gnome
4 OCHR, 8 SIŁ, 12 ZRE, 14 WOL, crossbow (k8, bulky)  
...
# Golem, Flesh
9 OCHR, 1 Pancerza, 15 SIŁ, 8 ZRE, 8 WOL, fists (k8)  
...
# Hooded Men  
12 OCHR, 9 SIŁ, 12 ZRE, 14 WOL, leystaff (k8), a Spellbook (Choose one: _Charm, Hypnotize, Push/Pull, Shield_)  
...
# Merman
4 OCHR, 12 SIŁ, 12 WOL, trident (k6)
...
# Nightmare
6 OCHR, 2 Pancerza, 14 SIŁ, 14 ZRE, 12 WOL, flaming hooves (k6+k6) or zęby (k8)
...
# Sphinx
12 OCHR, 12 SIŁ, 15 WOL, pazury (k6+k6) or roar (save)
...
-   Critical Damage: The poison paralyzes the victim (k8 ZRE damage),
    the spider drags paralyzed victims to its lair to eat them.

autor: [xenio](https://xenioinabottle.blogspot.com)

# Spider, Giant Aranea
3 OCHR, 12 ZRE, 14 WOL, zęby (k6), two random spellbooks
...
-   Regenerates 1 OCHR every turn. A Tarrasque always regenerates, even if
    its body is reduced to pieces. The way of permanently killing it is
    unknown.
-   Critical Damage: The razor-sharp pazury sever a part of the victim's
    body.

autor: [xenio](https://xenioinabottle.blogspot.com)

# Titan
20 OCHR, 2 Pancerza, 18 SIŁ, 12 ZRE, 14 WOL, sword (k10+k10)
...
8 OCHR, 1 Pancerza, 15 SIŁ, 8 ZRE, fists (k6+k6)
...
# Triton
4 OCHR, 1 Pancerza, 12 SIŁ, 12 ZRE, 12 WOL, trident (k6)
...
# Troll
6 OCHR, 1 Pancerza, 14 SIŁ, 12 ZRE, talons (k6+k6) or zęby (k8)
...
-   Can reach to zęby further targets due to its long neck.
...
-   Critical Damage: The Vampire drains the victim's essence (k12 WOL
    damage). At 0 WOL, the victim is killed and becomes a vampire.

autor: [xenio](https://xenioinabottle.blogspot.com)

# Warp Beast
6 OCHR, 14 ZRE, 12 WILL, tentacles (k6+k6)
...
# Werebear
6 OCHR, 14 SIŁ, 8 ZRE, pazury (k6+k6) or zęby (k8)
...
# Wereboar
4 OCHR, 13 SIŁ, 12 ZRE, 8 WOL, tusks (k6+k6)
...
# Wererat
3 OCHR, 8 SIŁ, 14 ZRE, 12 WOL, zęby (k4+k4) or weapon (k6)
...
# Weretiger
5 OCHR, 12 SIŁ, 14 ZRE, 12 WOL, pazury (k6+k6) or zęby (k8)
...
# Werewolf
4 OCHR, 12 SIŁ, 14 ZRE, 12 WOL, pazury (k4+k4) or zęby (k6)
...
-   When found in packs of at least 4, treat WOL as 15 for morale
    checks.

autor: [xenio](https://xenioinabottle.blogspot.com)

# Wood Troll
12 OCHR, 15 SIŁ, 12 ZRE, 7 WOL, pazury and zęby (k8+k8 blast)  
- As an action, can recover lost OCHR.   
- Critical damage: moss and twigs begin growing out of target’s wounds.   

source: [SRD](/cairn-srd#Bestiary)

# Wyvern
7 OCHR, 14 ZRE, 11 WOL, zęby (k8+k8) or poisonous sting (k6)
- Winged, two-legged, dragon-like monsters with a long tail tipped
    ...
    forests.
-   Critical Damage: The deadly poison inflicts k8 extra SIŁ damage.

autor: [xenio](https://xenioinabottle.blogspot.com)

# Yeti
4 OCHR, 14 SIŁ, 12 WOL, pazury (k6+k6) or paralyzing gaze (save)
-   Large (8’ tall), shaggy ape-like monsters with vicious pazury and
    a taste for human flesh. Dwell in isolated, icy regions.
-   Camouflage in the ice and snow to surprise its victims.
-   Gaze: surprised victims must save WOL or be paralyzed.

autor: [xenio](https://xenioinabottle.blogspot.com)

# Zombie
2 OCHR, 14 SIŁ, 6 ZRE, 3 WOL, weapon (k8)
